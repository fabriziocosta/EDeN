...
create_super_graph will eat a post_rna_struct object
	...
	if  optlist['abstr']:
		if has_structure(data.structure):
			...
			for stack in relevant:
				children.append( addabstr(G,stack[0],stack[1],optlist) )  # so we find out what the SOMETHINGS are and find abstract structures
		

			# addabstr gave is a bunch of nodes that represent a stack.... we need to connect the abstract stack representations.
...
def create_super_graph(data=None,optionlist=None):
	'''
	data is a post_rna_struct object...  created by call_rna_'programname'
	optionlist is a dictionary that was generated over argparse.. 
	
	the plan is this:
	we have created one or more post_rna_struct and we use
	create_super_graph(post_rna_struct,options) to get a networkx graph
	'''
	G=nx.Graph()
	G.add_node(0)
	G.node[0].update(data.attributes)
	
	
	G.node[0]['infonode']=True
	G.node[0]['supernde']=True

	for e in data.sequence_groups:
		newnode=len(G)
		G.add_edge(0,newnode,nesting=True)
		# copy annotations to node. 
		G.node[newnode].update(e.attributes)

		G.node[newnode]['infonode']=True
		for s in e.sequences:
			referencenode=addgraph(optionlist,s,G)
