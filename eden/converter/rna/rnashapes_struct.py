...
def rnashapes_wrapper(sequence, shape_type=None, energy_range=None, max_num=None):
    ...
    shape_list += [line.split()[2] for line in text[1:-1]] 
    #energy:
    energy_list = []
...
def string_to_networkx(header, sequence, **options):
    ...
    if split_components:
        for shape_str,energy_str,dotbracket_str in seq_struct_list:
            ...
            G.graph['info'] = 'RNAshapes shape_type=%s energy_range=%s max_num=%s shape=%s energy=%s dotbracket=%s' % (shape_type, energy_range, max_num, shape, energy, dotbracket)
